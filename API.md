...
- Set `GEMINI_API_KEY` to enable Gemini OCR. The default endpoint template is `https://api.hku.hk/gemini/student/{deployment-id}:generateContent`, and the default subscription-key header is `Ocp-Apim-Subscription-Key`.
